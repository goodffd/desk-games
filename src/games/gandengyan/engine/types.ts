...
/** 一个炸最多几张：一副牌里同点只有 4 张，王当替身也不许突破。 */
export const MAX_BOMB_SIZE = 4;

/**
 * 一张王代表哪张牌。
 *
 * **只带点数，不带花色**——干瞪眼没有同花牌型，`王 + 黑桃5` 当对 5 时，
 * 王算红桃 5 还是方块 5 毫无区别。花色进了指派，界面就会为了一个不影响任何事的
 * 选择去打断玩家。
 *
 * 点数只能是 3~A：**王不能替 2**。2 已经是特权牌（单张 2 压任意单张、对 2 压任意对子），
 * 再让王变 2，等于把两个逃生口叠在一张牌上。
 */
export interface WildAssign {
  /** 被指派的那张王的 id，必须确实在这次打出的牌里 */
  jokerId: number;
  /** 它代表的点数，3~A */
  rank: Rank;
}

/** 一手可以打出去的牌：牌 + 每张王的指派 + 它构成的牌型。 */
