...
- `/xiangqi`     → 象棋（大厅 SPA 前端路由 mount，不再单独服务 xiangqi-dist）
...
象棋已内置进 `dist/index.html` 单文件，部署只推 `dist/` + `server/`，不再单独推 `xiangqi-dist`；`server.mjs` 删除了 `/xiangqi/*` 静态路由（XQ_INDEX），需 restart 生效。
