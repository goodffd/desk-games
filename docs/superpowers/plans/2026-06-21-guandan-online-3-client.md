...
## 已知服务端缺口（owner 拍板项，本计划默认处理）

- **整盘重开（再来一盘）**：`guandan-rooms.mjs` 当前**无** `restart`/`again` 消息。→ 本计划 **Plan 3 不在客户端臆造**；联机 `matchOver` 弹层显示「整盘结束 + 名次 + 返回大厅」，不提供「再来一盘」。「留座重开」需服务端加 `restart` handler，**记入 Plan 4 待办**（或后续小补丁）。`?debug` 本地仍有「再来一盘」（LocalDriver.freshMatch）。
...
export interface GameDriver {
  ...
  freshMatch(): void;                // 本地：重开整盘。联机：no-op（matchOver 无再来一盘，见缺口）
...
3. `view.ts`：
   - `showResult(o: DealOutcome)`：读 `o.settle`（名次/升级文案，同前）+ `o.leftover`（末游剩牌，**不再读 state.hands**）。「下一局」按钮仅 `!driver.autoAdvance` 时显示→`driver.nextDealOrResult()`；「再来一盘」仅 `settle.match.over && !driver.autoAdvance`→`driver.freshMatch()`（联机 autoAdvance=true→无按钮，等 phase 变自动关）。
...
- Produces: `class OnlineDriver implements GameDriver`，构造 `new OnlineDriver(io: { on(type,cb); send(msg) }, mySeat: Seat | 'spectator')`。本任务实现：snapshot 拼装（含旋转）、phase、onChange/onResult/onSpeak、`autoAdvance=true`、`nextDealOrResult/freshMatch=no-op`、`start/timeoutSeat=no-op`（联机由服务端驱动；timeoutSeat 不发包，服务端有自己的托管/AI）。play/pass/tribute resolve 在 Task 5。
...
- [ ] **Step 1: 写失败单测**（mock io：`emit(type,msg)` 推消息、记录 send）`tests/online-driver.test.ts`：
  ...
  - `autoAdvance===true`；`nextDealOrResult()`/`freshMatch()` 不向 io.send 发任何东西。
...
- [ ] **Step 2:** RED。
- [ ] **Step 3:** 实现：`play(cards)`：`if (snapshot.turn!==0||phase!=='playing') return false; send(c2s.play(cards.map(c=>c.id))); return true`。`pass()` 同理（current!=null 守卫）。`need-tribute` handler 存 `myTributeOptions`；进 tribute state 时 fire 归一 onTribute（resolve→`send(c2s.tributeReturn(id))`，清 options）。`dispose()` 解绑 io。`timeoutSeat`/`start` no-op。
...
- **Spec 覆盖**：SPEC「客户端驱动层改造/前置 UI/协议/进贡还贡/观战/重连/掉线 AI/测试策略」逐项对应——协议(T1)、接缝归一(T2)、session 重连(T3)、OnlineDriver 映射+旋转(T4)+动作进贡(T5)、view 注入+?debug(T6)、UI 昵称/大厅/房间(T7-9)、控制器+观战+状态条(T10)、4-context 真机冒烟(T11)。「整盘重开」服务端缺口已显式记入 Plan 4 待办。多页 E2E 固化 + 部署属 Plan 4。
