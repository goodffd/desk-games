...
**Architecture:** 复用现有纯函数引擎（`src/games/guandan/engine` + `ai` + `match`，唯一真相，不双写）。`server/guandan-rooms.mjs` 管房间生命周期（纯 JS、无引擎依赖）；`server/guandan-match-driver.ts` 每个 playing 房一个权威牌局（import 引擎，按座位产出 outbound 消息）；现有合并 hub `server/server.mjs`（托管 大厅///xiangqi//ws象棋）只「扩展」加一个 `/ws-guandan` 挂掼蛋 RoomRegistry（跟象棋隔离，不动其余）。引擎+match-driver 经 esbuild 打成 `guandan-match-driver.bundle.mjs` 供生产 server 用；测试直接跑 TS。
...
├── rooms.mjs               # 既有(vendored 象棋 2 人 RoomRegistry)——不动
├── server.mjs              # 既有合并 hub(大厅///xiangqi//ws象棋)——本计划只「扩展」加 /ws-guandan
├── guandan-rooms.mjs       # 新：掼蛋 4 座房间登记（纯 JS，无引擎依赖）
├── guandan-match-driver.ts # 新：每房权威牌局（import 引擎/ai/match，产出 outbound）
└── build.mjs               # 新：esbuild 把 guandan-match-driver.ts(+引擎) 打成 guandan-match-driver.bundle.mjs
tests/
├── guandan-rooms.test.ts        # 新：掼蛋 RoomRegistry 房间生命周期（fakeClient）
└── guandan-match-driver.test.ts # 新：MatchDriver 引擎驱动 + 按座位下发 + AI + 进贡
.gitignore                  # 改：忽略 server/guandan-match-driver.bundle.mjs（构建产物）
...
- Modify: `.gitignore`（加 `server/guandan-match-driver.bundle.mjs`、`server/node_modules`）
...
```
# 联机服务端构建产物 / 依赖
server/guandan-match-driver.bundle.mjs
server/node_modules
```
...
**Files:**
- Create: `server/guandan-rooms.mjs`
- Create: `tests/guandan-rooms.test.ts`
...
- [ ] **Step 1: 写失败测试 `tests/guandan-rooms.test.ts`**
...
Run: `npm test -- guandan-rooms 2>&1 | tail -8`
Expected: FAIL — `Cannot find module '../server/guandan-rooms.mjs'`。

- [ ] **Step 3: 写 `server/guandan-rooms.mjs`（昵称部分）**
...
Run: `npm test -- guandan-rooms 2>&1 | tail -6`
Expected: PASS（4 个昵称用例）。

- [ ] **Step 5: Commit**

```bash
git add server/guandan-rooms.mjs tests/guandan-rooms.test.ts
...
**Files:**
- Modify: `server/guandan-rooms.mjs`
- Modify: `tests/guandan-rooms.test.ts`
...
**Files:**
- Create: `server/guandan-match-driver.ts`
- Create: `tests/guandan-match-driver.test.ts`
...
- [ ] **Step 1: 写失败测试 `tests/guandan-match-driver.test.ts`**
...
Run: `npm test -- guandan-match-driver 2>&1 | tail -8`
Expected: FAIL — 模块不存在。
...
Run: `npm test -- guandan-match-driver 2>&1 | tail -6`
Expected: PASS。

- [ ] **Step 5: Commit**

```bash
git add server/guandan-match-driver.ts tests/guandan-match-driver.test.ts
...
**Files:**
- Modify: `server/guandan-match-driver.ts`
- Modify: `tests/guandan-match-driver.test.ts`
...
- **Modify**: `server/server.mjs`（已存在的合并 hub：托管 `/` 大厅 + `/xiangqi` 象棋 + `/ws` 象棋联机。本任务**只新增**掼蛋联机，**绝不改动** `/`、`/xiangqi`、`/ws` 或 vendored 的 `server/rooms.mjs`）
- Modify: `package.json`（加 `"build:server": "node server/build.mjs"`）

**Interfaces:**
- Consumes: `server/guandan-rooms.mjs` + 打包后的 `server/guandan-match-driver.bundle.mjs`（导出 `MatchDriver`）。
- Produces: 现有 hub server.mjs 上**新增一个 `/ws-guandan` 的 WebSocketServer**挂掼蛋 `RoomRegistry`，每个 playing 房注入 `new MatchDriver(...)` + 还贡超时兜底定时器。象棋的 `/ws` 原封不动并存。
- **隔离要点（owner 拍板"跟象棋隔离"）**：掼蛋房间登记是 `guandan-rooms.mjs`（不碰 vendored `rooms.mjs`）；ws 路径是 `/ws-guandan`（不碰 `/ws`）。同一 http server 挂两个 `WebSocketServer`、各带不同 `path`，`ws` 按 path 各自只处理自己的 upgrade，互不干扰。
...
Run: `cd server && npm install --omit=dev 2>&1 | tail -2 && cd .. && node server/build.mjs && node -e "import('./server/guandan-match-driver.bundle.mjs').then(m=>console.log('export MatchDriver:', typeof m.MatchDriver))"`
Expected: 打印 `built ...` 和 `export MatchDriver: function`。

- [ ] **Step 3: 扩展现有 hub `server/server.mjs`（只加掼蛋 `/ws-guandan`，不动象棋）**

先 `Read` 现有 `server/server.mjs`（合并 hub：handler 路由 `/`→大厅、`/xiangqi`→象棋；`const reg = new RoomRegistry()` + `/ws` 象棋 wss；末尾 `server.listen(...)`）。**保持它原样**，只做两处新增：

(a) 在顶部已有 `import { RoomRegistry } from './rooms.mjs';` 之后，**新增两行 import**（用别名避免与象棋 `RoomRegistry` 冲突）：
```js
import { RoomRegistry as GuandanRooms } from './guandan-rooms.mjs';
import { MatchDriver } from './guandan-match-driver.bundle.mjs';
```

(b) 在现有象棋 `wss`（`new WebSocketServer({ server, path: '/ws', ... })` 那段）之后、`server.listen(...)` 之前，**新增掼蛋的 reg + 第二个 wss**：
```js
...
const gwss = new WebSocketServer({ server, path: '/ws-guandan', maxPayload: 1 << 20 });
...
> **不要重写整个 server.mjs**。只插入上面 (a)(b) 两段；`/`、`/xiangqi`、`/ws`、vendored `rooms.mjs` 一概不动。同一 http server 上挂两个 WebSocketServer、各带不同 `path`，`ws` 会按 path 各自只处理自己的 upgrade，互不干扰。
> **还贡超时**：`guandan-rooms.mjs` 构造收第三参 `tributeTimeoutMs`（这里传 `TRIBUTE_TIMEOUT=30000`）；在发出含 `need-tribute` 的 outbound 后启动 `setTimeout(()=>this._dispatch(room, room.driver.forceAutoReturn()), ms)`，收到 `tribute-return` 或还贡收齐（出现 `phase:'playing'` 的 state）后 `clearTimeout`（见 Step 4 的 `_armTributeTimeout`）。
...
Run（写 `/tmp/wssmoke.mjs` 用 `ws` 连 4 个客户端到 **`ws://localhost:8099/ws-guandan`** 建房开打，断言收到 `hand`/`state`）：
```bash
npm run build && node server/build.mjs && (PORT=8099 node server/server.mjs &) && sleep 1 && node /tmp/wssmoke.mjs
```
> 注：server.mjs 是合并 hub，启动需要 `dist/index.html`（`npm run build` 已生成）；`xiangqi-dist` 缺失不影响——只有请求 `/xiangqi` 才读它，本冒烟只连 `/ws-guandan`。
Expected: 4 客户端 hello→create/join/take-seat→start→各收到 `hand`(27) + `state`(playing)。脚本打印 `OK`。（脚本内容在 Plan 3/E2E 复用；此处最小验证 server 粘合通。）

- [ ] **Step 6: Commit**

```bash
git add server/build.mjs server/server.mjs server/guandan-rooms.mjs package.json
