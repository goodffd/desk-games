...
## 是什么
桌游合集：统一首页(游戏列表) → 点击进入具体游戏。首发**掼蛋**(内置)，**中国象棋**作为外链项。一期目标=掼蛋单局对 AI、规则零错。
...
│   │   ├── registry.ts        # GameModule 接口 + 内置/外链项登记
│   │   ├── home.ts            # 游戏列表首页 (DOM)
...
- 象棋外链 URL：**绝不硬编进公开仓库**（同 xiangqi 脱敏纪律）。用占位符 + 本地/部署时注入（如 `src/shell/links.local.ts` 加 .gitignore，或构建期环境变量）；仓库内只放占位。
...
- **公网信息脱敏**：推公开仓库前不得暴露象棋公网域名/端口/内网 IP（沿用 xiangqi 红线）。
